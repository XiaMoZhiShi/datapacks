...
advancement revoke @s only nifeather_extras:disguise/power_0

#add custom below

#0: ok, 1: err!
execute store success score @s nife_ods_timer run execute if entity @p[tag=!ods, distance=..20]
execute if score #LoadedImpersonate nife_stats matches 1 run function nifeather_extras:events/drink_swap/power/0/run
