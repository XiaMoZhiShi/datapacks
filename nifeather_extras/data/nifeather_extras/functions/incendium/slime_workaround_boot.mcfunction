...
execute if score slimes nife_stats >= slimecaps nife_stats in the_nether at @a[distance=0..] as @e[type=slime, distance=16..128, limit=10, sort=furthest] at @s run function nifeather_extras:incendium/slime_workaround_run