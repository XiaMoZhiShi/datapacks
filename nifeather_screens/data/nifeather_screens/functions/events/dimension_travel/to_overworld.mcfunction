...
execute unless entity @s[tag=screen_died] if predicate nifeather:is_day run title @s subtitle {"translate": "%1$s%3$s", "with": ["", [{"translate": "flat_world_preset.minecraft.overworld", "color": "#333333"}], ""]}
execute unless entity @s[tag=screen_died] if predicate nifeather:weather/thunder run title @s subtitle {"translate": "%1$s%3$s", "with": ["", [{"translate": "flat_world_preset.minecraft.overworld", "color": "#ffffff"}], ""]}
execute unless entity @s[tag=screen_died] if predicate nifeather:is_night run title @s subtitle {"translate": "%1$s%3$s", "with": ["", [{"translate": "flat_world_preset.minecraft.overworld", "color": "#ffffff"}], ""]}

effect give @s resistance 3 255 true
tag @s[tag=!joinEventProcessing] add suspendTeleportScreen
