...
execute if entity @s[tag=!old_impl_workaround] run function nifeather_3lives:events/player/join/old_impl_workdround
tag @s add old_impl_workaround

#补丁
execute unless score @s n3l_patchlevel matches 1.. run function nifeather_3lives:events/player/join/patches/1_subtitle
